...
<head>
    ...
    <title>@yield('title', 'Admin Dashboard') - {{ config('app.name', 'Exposia') }}</title>
...
<body class="h-full bg-gray-50 dark:bg-gray-900 transition-colors duration-300 font-sans">
    ...
    <script>
        ...
        window.confirmToggle = function(options = {}) {
            const isActivating = options.isActivating || false;
            const itemName = options.itemName || 'item';

            const toggleOptions = {
                title: isActivating ? 'Konfirmasi Aktivasi' : 'Konfirmasi Nonaktivasi',
                text: isActivating ? 
                    `Apakah Anda yakin ingin mengaktifkan ${itemName} ini?` : 
                    `Apakah Anda yakin ingin menonaktifkan ${itemName} ini?`,
                icon: isActivating ? 'question' : 'warning',
                showCancelButton: true,
                confirmButtonText: isActivating ? 'Ya, Aktifkan!' : 'Ya, Nonaktifkan!',
                cancelButtonText: 'Batal',
                confirmButtonColor: isActivating ? '#059669' : '#f59e0b',
                cancelButtonColor: '#6b7280',
                reverseButtons: true,
                focusCancel: true,
                ...options
        ...
        window.handleActionWithConfirmation = function(element) {
            ...
            const itemName = element.dataset.itemName || 'item';
            const isActivating = element.dataset.isActivating === 'true';
            ...
            switch (confirmType) {
                case 'delete':
                    ...
                    break;
                case 'toggle':
                    confirmFunction = window.confirmToggle;
