...
@section('content')
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-200 p-6 animate-slide-up">
                <div class="flex items-center mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Pilih desain yang sesuai dengan bisnis Anda</p>
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach ($templates as $template)
                        <div class="template-card group border-2 rounded-xl p-4 cursor-pointer transition-all duration-300 {{ $businessTemplate && $businessTemplate->template_id == $template->id ? 'border-gray-800 bg-gray-50' : 'border-gray-200 hover:border-gray-500' }}"
                            ...
                            @else
                                <div class="flex items-center text-gray-500 group-hover:text-gray-800 transition-colors">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-200 p-6 animate-slide-up">
                <div class="flex items-center mb-6">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">Palet Warna</h2>
                        <p class="text-sm text-gray-600">Sesuaikan warna sesuai brand Anda</p>
                    </div>
                </div>
                ...
                <div class="space-y-6 sm:space-y-4">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                        <label class="flex items-center text-sm font-medium text-gray-700 transition-colors duration-300 sm:w-32">
                            <i class="fas fa-circle mr-2 text-gray-600"></i>
                            Warna Aksen
                        </label>
                        <div class="flex items-center gap-3 flex-1">
                            <input type="color" id="accent-color"
                                value="{{ $businessTemplate ? $businessTemplate->getAccentColor() : '#012c4e' }}"
                                class="h-12 w-12 sm:w-16 rounded-lg border-2 border-gray-300 cursor-pointer">
                            <input type="text" id="accent-color-text"
                                value="{{ $businessTemplate ? $businessTemplate->getAccentColor() : '#012c4e' }}"
                                class="flex-1 w-full px-4 py-3 border-2 border-gray-300 rounded-xl bg-white text-gray-900 placeholder-gray-500 focus:border-gray-800 focus:ring-2 focus:ring-gray-800/20 focus:outline-none transition-all duration-200">
                        </div>
                    </div>
                ...
                <div class="mt-6">
                    <button onclick="updateColors()"
                        class="bg-gray-800 hover:bg-gray-700 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-200 transform hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 flex items-center space-x-2">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-200 p-6 animate-slide-up">
                <div class="flex items-center mb-6">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">Konfigurasi Section</h2>
                        <p class="text-sm text-gray-600">Pilih section dan style untuk setiap bagian website</p>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    @foreach ($availableSections as $sectionKey => $sectionConfig)
                        ...
                        <div class="section-config-item border-2 border-gray-200 rounded-xl p-5 {{ $isActive ? 'bg-gray-50 border-gray-400' : '' }} transition-all duration-300">
                            <!-- Section Header -->
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center mr-3">
                                        <i class="fas fa-{{ getSectionIcon($sectionKey) }} text-gray-600"></i>
                                    </div>
                                    <div>
                                        <h3 class="font-semibold text-gray-900">{{ $sectionConfig['name'] }}</h3>
                            ...
                            <div class="style-variants {{ $isActive ? '' : 'hidden' }}" id="variants-{{ $sectionKey }}">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    @foreach ($sectionConfig['variants'] as $variantKey => $variantName)
                                        <div class="variant-option cursor-pointer border-2 rounded-lg p-3 transition-all duration-200 {{ $currentVariant == $variantKey ? 'border-gray-600 bg-gray-50' : 'border-gray-200 hover:border-gray-400' }}"
                                            data-section="{{ $sectionKey }}" data-variant="{{ $variantKey }}"
                                            onclick="updateSectionStyle('{{ $sectionKey }}', '{{ $variantKey }}')">

                                            <div class="flex items-center justify-between mb-2">
                                                <span class="font-medium text-gray-900">Style {{ $variantKey }}</span>
                                                @if ($currentVariant == $variantKey)
                                                    <i class="fas fa-check-circle text-gray-600"></i>
                                                @endif
                                            </div>

                                            <p class="text-sm text-gray-600">{{ $variantName }}</p>

                                            <!-- Style Preview Button -->
                                            <div class="mt-2">
                                                <button type="button"
                                                    class="preview-button w-full h-12 bg-gradient-to-r from-gray-100 to-gray-200 rounded border flex items-center justify-center hover:from-gray-200 hover:to-gray-300 transition-colors duration-200"
        ...
        <div class="lg:col-span-1 space-y-8">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-200 p-6 animate-slide-up sticky">
                <div class="flex items-center mb-6">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">Gambar Hero</h2>
                        <p class="text-sm text-gray-600">Gambar utama website</p>
                    </div>
                </div>
                ...
                @if ($business->hero_image_url)
                    <div class="current-hero mb-6">
                        <div class="text-center">
                            <div class="aspect-video bg-white rounded-xl border-2 border-gray-200 overflow-hidden shadow-lg">
                                <img src="{{ asset('storage/' . $business->hero_image_url) }}" alt="Current Hero"
                                    class="w-full h-full object-cover">
                            </div>
                            <p class="text-sm text-gray-600 mt-2">Gambar hero saat ini</p>
                        </div>
                    </div>
                @endif

                <!-- Hero Image Upload Area -->
                <div class="hero-upload-dropzone relative border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-gray-500 transition-all duration-300 cursor-pointer group"
                    id="hero-dropzone">
                    ...
                    <div class="upload-progress hidden" id="hero-upload-progress">
                        <div class="w-16 h-16 mx-auto mb-4">
                            <div class="animate-spin rounded-full h-16 w-16 border-b-2 border-gray-600"></div>
                        </div>
                ...
                <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-200 p-6 animate-slide-up mt-6">
                    <div class="flex items-center mb-6">
                        <div class="w-10 h-10 bg-gray-500 rounded-lg flex items-center justify-center mr-4">
                            <i class="fas fa-images text-white"></i>
                        </div>
                        <div>
                            <h2 class="text-lg font-semibold text-gray-900">Gambar Hero Kedua</h2>
                            <p class="text-sm text-gray-600">Gambar pendukung untuk hero section</p>
                        </div>
                    </div>
                    ...
                    @if ($business->hero_image_secondary_url)
                        <div class="current-secondary-hero mb-6">
                            <div class="text-center">
                                ...
                                <p class="text-sm text-gray-600 mt-2">Gambar hero kedua saat ini</p>
                            </div>
                        </div>
                    @endif

                    <!-- Secondary Hero Image Upload Area -->
                    <div class="secondary-hero-upload-dropzone relative border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-gray-500 transition-all duration-300 cursor-pointer group {{ $business->hero_image_secondary_url ? 'hidden' : '' }}"
            ...
            <div class="flex flex-col gap-4 animate-slide-up">
                <a href="{{ route('user.dashboard') }}"
                    class="w-full bg-gray-100 hover:bg-gray-200 text-gray-900 font-semibold py-4 px-6 rounded-xl transition-all duration-200 transform hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 text-center flex items-center justify-center space-x-2">
    ...
    <div id="preview-modal"
        ...
        <div class="relative bg-white rounded-xl shadow-xl max-w-3xl w-full mx-auto transform scale-95 transition-all duration-300 overflow-y-auto">
            <!-- Modal Header -->
            <div class="flex items-center justify-between p-4 border-b border-gray-200">
                <h3 class="text-xl font-semibold text-gray-900" id="preview-modal-title">Preview Style</h3>
                <button type="button"
            ...
            <!-- Modal Body -->
            <div class="p-6">
                <div class="max-h-96 overflow-y-auto bg-gray-100 rounded-lg mb-4">
                    <img id="preview-modal-image" src="" alt="Style Preview"
                        class="w-full object-contain mx-auto" />
                </div>

                <p class="text-gray-600 text-sm">(scrollable)</p>
                <p id="preview-modal-description" class="text-gray-600 text-sm"></p>
            </div>
            <!-- Modal Footer -->
    ...
    @push('scripts')
        <script>
            ...
            function initializeColorPickers() {
                ['primary', 'secondary', 'accent', 'highlight'].forEach(type => {
                ...
                document.querySelectorAll('.section-toggle').forEach(toggle => {
                    toggle.addEventListener('change', function() {
                        ...
                        if (isChecked) {
                            ...
                            sectionItem.classList.add('bg-gray-50', 'border-gray-400');
                            sectionItem.classList.remove('border-gray-200');
                        } else {
                            variantsDiv.classList.add('hidden');
                            sectionItem.classList.remove('bg-gray-50', 'border-gray-400');
                            sectionItem.classList.add('border-gray-200');
                        }
                    });
                });
            }
            ...
            function updateSectionStyle(section, styleVariant) {
                document.querySelectorAll(`[data-section="${section}"].variant-option`).forEach(option => {
                    option.classList.remove('border-gray-600', 'bg-gray-50');
                ...
                if (selectedOption) {
                    ...
                    selectedOption.classList.add('border-gray-600', 'bg-gray-50');
                    ...
                    if (titleDiv && !titleDiv.querySelector('.fa-check-circle')) {
                        ...
                        checkIcon.className = 'fas fa-check-circle text-gray-600';
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            <div class="text-center">
                                ...
                                <div class="mt-4 flex justify-center space-x-3">
                                    <button type="button" onclick="document.getElementById('hero-image-secondary').click()" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg transition-colors duration-200">
                                        <i class="fas fa-edit mr-2"></i>Ganti Foto
                                    </button>
