...
<div class="mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-slide-up">
                ...
                <div class="px-6 py-4 border-b border-gray-200">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h2 class="text-lg font-semibold text-gray-900">Status Website</h2>
                            <p class="text-sm text-gray-600">Status publikasi website Anda</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="lg:col-span-2">
                            <div class="p-4 rounded-lg {{ $business->isPublished() ? 'bg-green-50 border border-green-200' : 'bg-gray-50 border border-gray-200' }}">
                                <div class="flex items-start">
                                    <div class="w-8 h-8 rounded-full {{ $business->isPublished() ? 'bg-green-100' : 'bg-gray-200' }} flex items-center justify-center mr-3 mt-0.5">
                                        <i class="fas {{ $business->isPublished() ? 'fa-check text-green-600' : 'fa-clock text-gray-500' }} text-sm"></i>
                                    </div>
                                    <div class="flex-1">
                                        <h3 class="font-semibold {{ $business->isPublished() ? 'text-green-800' : 'text-gray-700' }} text-lg">
                                            {{ $business->isPublished() ? 'Aktif' : 'Draft' }}
                                        </h3>
                                        <p class="text-sm {{ $business->isPublished() ? 'text-green-700' : 'text-gray-600' }} mt-1">
                                        ...
                                        <div class="mt-3 space-y-1">
                                            ...
                                            @endif
                                            @else
                                            <div class="flex items-center text-yellow-600 text-xs font-medium">
                        ...
                        <div class="lg:col-span-1 flex items-center">
                            <form id="toggleStatusForm" action="{{ route('user.publish.toggle-status') }}" method="POST" class="w-full">
                                ...
                                <button type="submit" {{ !$business->isReadyToPublish() && !$business->isPublished() ? 'disabled' : '' }} class="w-full px-6 py-3 rounded-lg font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all duration-200
                                    {{ $business->isPublished() 
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-slide-up">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <!-- Current URL Display -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-3">URL Aktif:</label>
                            @if($business->public_url)
                            <div class="p-4 rounded-lg bg-gray-50 border border-gray-200">
                                <div class="flex items-center justify-between">
                                    <div class="flex-1 min-w-0">
                                        <a href="{{ $business->public_url }}" target="_blank" class="text-gray-700 hover:text-gray-900 hover:underline font-medium text-sm break-all">
                                            {{ $business->public_url }}
                                        </a>
                                    </div>
                                    @if($business->isPublished())
                                    <a href="{{ $business->public_url }}" target="_blank" class="ml-3 p-2 text-gray-600 hover:text-gray-800 flex-shrink-0">
                            ...
                            <div class="p-4 rounded-lg bg-yellow-50 border border-yellow-200">
                                <div class="flex items-center">
                                    ...
                                    <p class="text-yellow-700 text-sm">URL website belum diatur</p>
                                </div>
                            </div>
                            @endif
                        </div>
                        ...
                        <div>
                            <form id="updateUrlForm" action="{{ route('user.publish.update-url') }}" method="POST" class="space-y-4">
                                @csrf
                                <div>
                                    <label for="url_slug" class="block text-sm font-medium text-gray-700 mb-3">
                                        Atur URL Website
                                    </label>
                                    <div class="flex">
                                        ...
                                        <input type="text" name="url_slug" id="url_slug" value="{{ $business->public_url ? basename(parse_url($business->public_url, PHP_URL_PATH)) : '' }}" class="flex-1 min-w-0 block px-4 py-2.5 rounded-r-lg border border-gray-300 bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-800 focus:border-gray-800 @error('url_slug') border-red-500 @enderror" placeholder="nama-usaha-anda">
                                    </div>
                                    @error('url_slug')
                                ...
                                <button type="submit" class="w-full px-5 py-2.5 bg-gray-800 hover:bg-gray-700 text-white font-medium rounded-lg shadow-sm hover:shadow focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transform hover:scale-[1.02] transition-all duration-200">
        ...
        <div class="xl:col-span-1">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-slide-up sticky top-6">
                <!-- Card Header -->
                <div class="px-6 py-4 border-b border-gray-200">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Akses cepat website</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6">
                    @if($business->public_url)
                    <div class="space-y-4">
                        <div class="w-full aspect-square bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
                        ...
                        <div class="space-y-2">
                            <a href="{{ route('user.publish.download-qr') }}" class="w-full inline-flex items-center justify-center px-4 py-2.5 bg-gray-700 hover:bg-gray-600 text-white font-medium rounded-lg shadow-sm hover:shadow focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transform hover:scale-[1.02] transition-all duration-200">
